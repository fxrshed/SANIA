...
def main(model_name: str, dataset: str, optimizer: str, epochs: int, train_batch_size: int, test_batch_size: int, 
    ...
    elif model_name == "LeNet5X":
        model = LeNet5X().to(device)

    criterion = nn.CrossEntropyLoss()
    
    if dataset == "MNIST":
        train_loader, test_loader = get_MNIST(train_batch_size, test_batch_size, scale, seed=0)
    ...
    if save:
        save_results(result, model_name, dataset, scale, train_batch_size, epochs, optimizer, lr, seed=seed)
...
if __name__ == "__main__":
    ...
    for seed in range(args.seeds):
        print(f"Seed: {seed}")
        main(model_name=args.model, dataset=args.dataset, optimizer=args.optimizer, epochs=args.epochs, 
